        ...
        return this.each(function()
        {
            ...
            function initSudoSlider(obj, destroyT)
            {
                ...
                if (option[25]/*ajax*/)
                {
                    ...
                    if (ul.length == 0)
                        obj.append(ul = $('<ul></ul>'));// No it's not, lets create it. 

                    // Do we have enough list elements to fill out all the ajax documents. 
                    if (option[25]/*ajax*/.length > s)
                    {
                        // No we dont. 
                        for (a = 1; a <= option[25]/*ajax*/.length - s; a++)
                            ul.append('<li><p>' + option[35]/*loadingText*/ + '</p></li>');
                        li = ul.children("li");
                        s = li.length;
                    }
                }
                ...
                if (obj.css("position") == "static")
                    obj.css("position", "relative"); // Fixed a lot of IE6 + IE7 bugs. 
                ...
                if (!option[21]/*fade*/)
                    option[40]/*slideCount*/ += option[42]/*moveCount*/;
                ...
                if (continuousClones)
                {
                    ...
                    if (option[25]/*ajax*/)
                    {
                        ...
                        {
                            if (option[25]/*ajax*/[a] && a != option[27]/*startSlide*/ - 1)
                                ajaxLoad(a, falsev, 0, falsev); // I still do not wan't to load the current slide at this point. 
                        }
                    }
                ...
                if (option[26]/*preloadAjax*/ === truev)
                    for (i = 0; i <= ts; i++)
                        if (option[25]/*ajax*/[i] && option[27]/*startSlide*/ - 1 != i)
                            ajaxLoad(i, falsev, 0, falsev);
                ...
                runOnImagesLoaded(liConti.slice(0, option[40]/*slideCount*/), truev, function()
                {
                    ...
                    // startSlide is allways enabled, if not by the user, then by the code. 
                        ...
                        animate(option[27]/*startSlide*/ - 1, falsev, falsev, falsev);
                });
            }
            /*
            ...
            function URLChange()
            {
                ...
                if (init)
                    animate(i, falsev, falsev, falsev);
                else if (i != t)
                    goToSlide(i, falsev);
            }
            ...
            function goToSlide(i, clicked, speed)
            {
                ...
                if (!destroyed)
                {
                    ...
                    if (option[9]/*auto*/)
                    {
                        ...
                        if (fading && option[22]/*crossFade*/)
                            delay = parseInt10((delay) * (3 / 5));
                        else if (fading)
                            delay = 0;
                        // Stopping auto if clicked. And also continuing after X seconds of inactivity. 
                        if (clicked) {
                            stopAuto();
                            if (option[41]/*resumePause*/)
                                timeout = startAuto(delay + option[41]/*resumePause*/);
                        }
                        // Continuing if not clicked.
                        else
                            timeout = startAuto(option[10]/*pause*/ + delay);
                    }
            ...
            function fadeControl(fadeOpacity, fadetime, nextcontrol) // It may not sound like it, but the variable fadeOpacity is only for truev/falsev. 
            {
                ...
                {
                    if (option[12]/*prevNext*/)
                        eA[fadeOpacity ? 'fadeIn' : 'fadeOut'](fadetime);
                    if (firstlastshow)
                        eB[fadeOpacity ? 'fadeIn' : 'fadeOut'](fadetime);
                }
                if (option[20]/*customLink*/)
                {
            ...
            function setCurrent(i)
            {
                i = getRealPos(i) + 1;
                if (option[13]/*numeric*/)
                    for (a in numericControls)
                        setCurrentElement(numericControls[a], i);
                if (option[20]/*customLink*/)
                    setCurrentElement($(option[20]/*customLink*/), i);
            }
            ;
            function setCurrentElement(element, i)
            {
                if (element.filter)
                {
                    element
                            ...
                            .removeClass("current")
                            .each(function() {
                        if (isFunc(option[31]/*uncurrentFunc*/)) {
                            option[31]/*uncurrentFunc*/.call(this, $(this).attr("rel"));
                        }
                    });
                    ...
                    element
                        ...
                        return falsev;
                            ...
                            .addClass("current")
                            .each(function(index) {
                        if (isFunc(option[32]/*currentFunc*/)) {
                            option[32]/*currentFunc*/.call(this, i);
                        }
                    });
                }
            }
            ...
            function autoadjust(i, speed)
            {
                ...
                if (option[19]/*autoheight*/)
                    autoheightwidth(i, speed, truev);//autoheight(i, speed);
                if (option[39]/*autowidth*/)
                    autoheightwidth(i, speed, falsev);//autowidth(i, speed);
            }
            ...
            function adjustPosition() // høns
            {
                ...
                if (!option[6]/*vertical*/)
                    ul.css("margin-left", "-" + bigNumber + "px");
            ...
            function adjust()
            {
                ...
                if (!option[24]/*updateBefore*/)
                    setCurrent(t);
                adjustPosition();
                clickable = truev;
                if (option[17]/*history*/ && buttonclicked)
            ...
            {
                if (isFunc(option[30]/*afterAniFunc*/))
                    option[30]/*afterAniFunc*/.call(el, a);
            }
            function beforeAniCall(el, a)
            {
                if (isFunc(option[29]/*beforeAniFunc*/))
            ...
            {
                if (asyncTimedLoad)
                    clearTimeout(asyncTimedLoad);// I dont want it to run to often. 
            ...
            function ajaxAdjust(i, speed, ajaxCallBack, adjust, ajaxInit, img) {
                ...
                if (continuousClones)
                {
                    ...
                    {
                        if (notFirst)
                            callBackList[i][a].replaceWith($(target).clone());
                        notFirst = truev;
                    }
                ...
                runOnImagesLoaded(target, truev, function() {
                    if (ajaxInit === truev)
                        adjustPosition();// Doing this little trick after the images are done. 
                    // And the callback. 
                    if (isFunc(ajaxCallBack))
                        ajaxCallBack();
                ...
                if (isFunc(option[28]/*ajaxLoadFunction*/)) {
                    option[28]/*ajaxLoadFunction*/.call(target, parseInt10(i) + 1, img);
                }
            ...
            function fadeto(i, clicked, ajaxcallback)
            {
                if (filterDir(i) != t && !destroyed && clickable) // We doesn't want something to happen all the time. The URL can change a lot, and cause som "flickering". 
                {
                    ...
                    if (option[2]/*controlsFade*/)
                        fadeControls(ll, option[1]/*controlsFadeSpeed*/);
                    ...
                    if (ajaxcallback)
                    {
                        ...
                        if (dontCountinueFade)
                            dontCountinueFade--; // It is nice that 0 == falsev;
                    }
                    ...
                    if (!dontCountinueFade) // if (dontCountinueFade == 0)
                    {
                        ...
                        if (option[22]/*crossFade*/)
                        {
                            ...
                            for (a = ll; a < ll + orgSlideCount; a++)
                            {
                                ...
                                if (isFunc(option[29]/*beforeAniFunc*/))
                                    option[29]/*beforeAniFunc*/.call(clone, getRealPos(a) + 1);
                                clone.css({'z-index': '10000', 'position': 'absolute', 'list-style': 'none', 'top': option[6]/*vertical*/ ? push : 0, 'left': option[6]/*vertical*/ ? 0 : push}).
                                    ...
                                    if (firstRun)
                                    {
                                        ...
                                        if (option[17]/*history*/ && clicked)
                                            window.location.hash = option[15]/*numericText*/[t]; // It's just one line of code, no need to make a function of it. 
            ...
            function animate(dir, clicked, time, ajaxcallback, speed) // (Direction, did the user click something, is this to be done in >1ms?, is this inside a ajaxCallBack?) 
            {
                if ((clickable && !destroyed && (dir != t || init)) && s > getRealPos(dir) || ajaxcallback)
                {
                    ...
                    if (!(speed || speed == 0))
                        var speed = (!time) ? 0 : ((!clicked && !option[9]/*auto*/) ? parseInt10(diff * option[18]/*speedhistory*/) : parseInt10(diff * option[7]/*speed*/)),
                            // Ajax begins here 
                    ...
                    if (ajaxcallback)
                    {
                        ...
                        // Do a check if it can continue.
                        if (dontCountinue)
                            dontCountinue--; // It is nice that 0 == falsev;
                    }
                    else if (option[25]/*ajax*/)
                    {
                    ...
                    if (!dontCountinue)
                    {
                        ...
                        ul.animate(
                                ...
                                if (option[2]/*controlsFade*/)
                                {
                                    ...
                                    if (!clicked && !option[9]/*auto*/)
                                        fadetime = (option[18]/*speedhistory*/ / option[7]/*speed*/) * option[1]/*controlsFadeSpeed*/;
                                    if (!time)
                                        fadetime = 0;
                                    if (fading)
                                        fadetime = parseInt10((option[23]/*fadespeed*/) * (3 / 5));
            ...
            baseSlider.insertSlide = function(html, pos, numtext) {
                ...
                if (pos > s)
                    pos = s; // If you try to add a slide after the last slide fix. 
                var html = '<li>' + html + '</li>';
                if (!pos || pos == 0)
                    ul.prepend(html);
                else
            ...
            baseSlider.destroy = function() {
                ...
                if (continuousClones)
                    for (a = 1; a <= option[40]/*slideCount*/; a++)
                        liConti.eq(a - 1).add(liConti.eq(-a)).remove();
